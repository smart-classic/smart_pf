...
var PFState = {LoggedIn: false, PatientList: {}};
...
var panel = require("panel").Panel({
    ...
    onShow: function(){
	console.log("showing panel: " + JSON.stringify(PFState.PatientList));	
	...
	this.postMessage({LoggedIn: PFState.LoggedIn, PatientList: PFState.PatientList});
    },
	...
	{
	    windows.open(data.url('pf_container.html')+"?app="+
...
pageMod.PageMod({
  include: data.url('pf_container.html')+'*',
  contentScriptWhen: 'ready',
...
PFListener.prototype = {
	...
	os.add("http-on-examine-response", function(s,d) {
		...
		console.log("Parsed replydom");
		var pfResponse = replydom.documentElement.firstElementChild.firstElementChild;
		console.log("Parsed replydom as: " + pfResponse);
		console.log("Parsed replydom namde: " + pfResponse.nodeName);

		if (PF[pfService] && PF[pfService].wsdl.types[pfResponse.nodeName]) {
		    console.log("we can handle " + pfService + " . " + pfResponse.nodeName);
		    var jspayload = PF[pfService].interface.parse_type(pfResponse.nodeName, pfResponse);
		    console.log(JSON.stringify(jspayload, null, 2));
		    console.log(".. and that was it.");
		}
		else {
		    console.log("no handle " + pfService + " . " + pfResponse.nodeName);
		}
		
		console.log("PArsed with dom parser");
		...
		console.log(replydom.childNodes[0].childNodes[0].childNodes[0]);
		console.log("PF ");
		console.log(replydom.childNodes[0].childNodes[0].childNodes[0].nodeName);
...
LoginListener.prototype.httpCallback = function(responseXML, postBody) {
	...
	for each(var p in patients) {
			...
			if (PFState.PatientList[pobj.id] === undefined) {
				PFState.PatientList[pobj.id] = pobj;
			}			
	}
